...
VAR_std = function(x, configuration=list(nlag=1)) {
  if (!is(x, "zoo")) {
  ...
  for (i in 1:k) {
    ...
    #std_Res = cbind(std_Res, fit$residual/se)
  }
  Q = array(t(Res)%*%Res/nrow(Res), c(k, k, 1), dimnames=list(NAMES, NAMES, tail(as.character(zoo::index(x)),1)))
  std_Q = matrix(Q, 52, 52)/se
  #std_Q=1
  results = list(B=B, Q=Q, std_Q=std_Q, se=se)
}
